#Updates CLoudflare DNS using Cloudflare API and AWS checkip to get current ip, supports multiple records
...
record_identifier_2 = "your-second-dns-record-identifier"
record_type = "A"
ip_old = "0.0.0.0"
...
def updateIP(ip):
    ...
    data2 = json.dumps({"content": ip, "name": "second-host-name", "proxied": False, "type": "A", "comment": "Main record", "ttl": 1})
    answer2 = client.put(f"https://api.cloudflare.com/client/v4/zones/{zone_identifier}/dns_records/{record_identifier_2}", data=data2, headers=headers)
...
def run():
    ...
    if ip != ip_old:
        ...
        with open(os.path.join(FOLDER, log_file), "a") as log:
            log_dict = json.dumps({"oldIP": ip_old, "newIP": ip, "date": dt.datetime.now().strftime("%m/%d/%Y, %H:%M:%S"), "answer": f"{answers[0].text}; {answers[1].text}" })
